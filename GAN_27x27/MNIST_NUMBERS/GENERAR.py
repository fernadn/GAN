...
def sample_single_image(generator, img_shape, etiqueta=1, fig_size=(2, 2)):
    noise = np.random.normal(0, 1, (1*1, 100))
    gen_imgs = generator.predict(noise)   # Genera imagen
    # Rescale images 0 - 255
    gen_imgs = 255*(0.5 * gen_imgs + 0.5)
    ...
    plt.axis('off')
    plt.savefig('images_model/imagen_%d.png' % etiqueta)
    plt.show()
    plt.close()
...
if __name__ == '__main__':
    ...
    # sample_single_image(generador, img_shape, etiqueta=2) # Genera imagen individual
